...
<style>body{font-family:Arial,sans-serif;background:#10141c;color:#f5f7fa;margin:0}.wrap{max-width:1050px;margin:auto;padding:32px}.card{background:#1b2230;border:1px solid #303a4d;border-radius:16px;padding:22px;margin:18px 0}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:14px}input,select{width:100%;box-sizing:border-box;padding:11px;border-radius:9px;border:1px solid #49556c;background:#10141c;color:white}button,.button{display:inline-block;padding:11px 16px;border:0;border-radius:9px;font-weight:700;cursor:pointer;text-decoration:none}.primary{background:white;color:#10141c}.green{background:#4ade80;color:#102016}.berry{background:#8b2b52;color:white}.tiktok{background:#25f4ee;color:#101014}.muted{color:#aab3c2;font-size:14px}.post{padding:18px 14px;border-top:1px solid #313a4b}.badge{padding:4px 8px;border-radius:20px;background:#303a4d;font-size:12px}.sharekit{margin-top:14px;padding:14px;border-radius:12px;background:#151b27;border:1px solid #303a4d}.sharekit button,.sharekit a{margin:4px 8px 4px 0}a{color:#b9d5ff}.copybox{position:absolute;left:-9999px}.notice{padding:10px 12px;border-left:4px solid #25f4ee;background:#202536;color:#d8deea;font-size:13px}</style></head><body><div class=wrap><h1>Invome Content Autopilot</h1><p class=muted>Generate → make media → publish automatically.</p>
...
<div class=card><h2>Recent posts</h2>{% for p in posts %}<div class=post><b>{{p.title}}</b> <span class=badge>{{p.status}}</span> <span class=badge>{{'video' if p.media_file and p.media_file.endswith('.mp4') else 'image'}}</span><p>{{p.caption}}</p>{% if p.media_file %}<a href="/media/{{p.media_file}}" target=_blank>Preview {{'video' if p.media_file.endswith('.mp4') else 'image'}}</a><div class="sharekit"><b>Facebook Group Sharing Kit</b><p class=muted>Facebook requires group posts to be shared manually. Copy the caption, download the media, then post them in an approved group.</p><textarea class=copybox id="group-{{p.id}}">{{p.caption}}</textarea><button type=button class=berry onclick="copyText('group-{{p.id}}',this,'Copy group caption')">Copy group caption</button><a class="button primary" href="/media/{{p.media_file}}" download>Download {{'video' if p.media_file.endswith('.mp4') else 'image'}}</a></div><div class="sharekit"><b>TikTok Sharing Kit</b><p class=muted>Review the post, copy the shorter TikTok caption, download the media, and finish the upload in TikTok.</p><textarea class=copybox id="tiktok-{{p.id}}">{{p.hook}} Handmade sellers: know your real costs, price with confidence, and spend more time creating. Start your 7-day free trial—no card needed. {{s.site_url}} #HandmadeBusiness #CraftTok #InvoMeStudio</textarea><button type=button class=tiktok onclick="copyText('tiktok-{{p.id}}',this,'Copy TikTok caption')">Copy TikTok caption</button><a class="button primary" href="/media/{{p.media_file}}" download>Download {{'video' if p.media_file.endswith('.mp4') else 'image'}}</a><a class="button tiktok" href="https://www.tiktok.com/upload" target=_blank rel="noopener">Open TikTok Upload</a><p class=notice>Before posting, turn on <b>Content disclosure</b> and select <b>Your brand</b>. Review the caption and privacy setting, then publish manually.</p></div>{% endif %}{% if p.error %}<p style="color:#fca5a5">{{p.error}}</p>{% endif %}</div>{% else %}<p class=muted>No posts yet.</p>{% endfor %}</div>
</div><script>async function copyText(id,btn,label){const text=document.getElementById(id).value;try{await navigator.clipboard.writeText(text)}catch(e){const box=document.getElementById(id);box.style.position='static';box.select();document.execCommand('copy');box.style.position='absolute'}btn.textContent='Caption copied';setTimeout(()=>btn.textContent=label,1800)}</script></body></html>'''
